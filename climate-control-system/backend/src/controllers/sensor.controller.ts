...
import {
  fetchLastIngestStatusBySerial,
  fetchLatestReadings,
  fetchReadingsByDevice,
  ingestSensorData
...
export const statusBySerial = asyncHandler(async (req: Request, res: Response) => {
  const data = await fetchLastIngestStatusBySerial(req.params.deviceSerial);
